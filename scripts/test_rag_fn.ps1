# Test Search-BisKnowledgeChunks function
$root = $PSScriptRoot
...
$catJson = Get-Content -Raw $catLookupFile | ConvertFrom-Json
...
function Search-BisKnowledgeChunks($q, $topK = 4) {
        ...
        foreach ($prod in $global:BIS_PRODUCT_TAXONOMY) {
            ...
            if (-not $prodMatched -and $prod.hindi_terms) {
                foreach ($hTerm in $prod.hindi_terms) {
                    if ($cleanQLower.Contains($hTerm.ToLower())) {
                        $prodMatched = $true
                        break
                    }
                }
    ...
    if ($global:BIS_RAG_CHUNKS) {
        ...
        $digitsInQuery = @()
        [regex]::Matches($q, '\b\d{3,5}\b') | ForEach-Object { $digitsInQuery += $_.Value }
...
$res = Search-BisKnowledgeChunks "What are helmet shock absorption limits in IS 4151?"
$sw.Stop()
Write-Host "Search-BisKnowledgeChunks completed in $($sw.ElapsedMilliseconds)ms! Top standard: $($res.results[0].chunk.standardCode)"
